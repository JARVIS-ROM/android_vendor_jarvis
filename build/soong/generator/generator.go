...
// Copyright (C) 2018,2021 The JARVIS Project
//
...
func init() {
	android.RegisterModuleType("jarvis_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = jarvisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = jarvisExpandVariables(ctx, dep_file)
	...
	cmd := jarvisExpandVariables(ctx, String(g.properties.Cmd))
