function __print_jarvis_functions_help() {
cat <<EOF
Additional JARVIS functions:
...
- jarvisremote:   Add git remote for JARVIS github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"JARVIS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.jarvis.device | grep -q "$JARVIS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $JARVIS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function jarvisremote()
{
    ...
    git remote rm jarvis 2> /dev/null
    local REMOTE=$(git config --get remote.github.projectname)
    REMOTE=$(git config --get remote.aosp.projectname)
    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")
    git remote add jarvis git@github.com:JARVIS-ROM/$PROJECT
    echo "Remote 'jarvis' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.jarvis.device | grep -q "$JARVIS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        jarvisremote
        git push jarvis HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.jarvis.device | grep -q "$JARVIS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
